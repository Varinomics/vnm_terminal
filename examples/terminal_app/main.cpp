...
void apply_terminal_shell_geometry(
    ...
    chrome::Example_window_chrome* titlebar,
    bool                           custom_titlebar)
{
...
std::vector<QRectF> window_chrome_button_rects(
    const chrome::Example_window_chrome& titlebar)
{
...
void print_error(const QString& message)
{
    ...
    std::cerr << "vnm_terminal_example_terminal: " << bytes.constData() << '\n';
}

void print_usage()
{
    std::cout
        << "usage: vnm_terminal_example_terminal [options]\n"
        << "       vnm_terminal_example_terminal [options] -- <program> [args...]\n"
        << "\n"
        << "options:\n"
...
Parse_result parse_arguments(const QStringList& arguments)
{
    ...
    while (index < arguments.size()) {
        ...
        if (argument_is(argument, "--custom-titlebar")) {
            if (!custom_titlebar_supported_on_platform()) {
                result.error = QStringLiteral(
                    "--custom-titlebar is supported only after platform validation");
                return result;
            }

            result.options.custom_titlebar = true;
            ++index;
            continue;
        }
...
#ifndef VNM_TERMINAL_EXAMPLE_TERMINAL_NO_MAIN
int main(int argc, char** argv)
{
    ...
    QCoreApplication::setApplicationName(QStringLiteral("vnm_terminal_example_terminal"));
    const QIcon app_icon(
        QStringLiteral(
            ":/vnm_terminal_example_terminal/vnm_terminal.ico"));
    ...
    auto* titlebar = options.custom_titlebar
        ? new chrome::Example_window_chrome(window.contentItem())
        : nullptr;
