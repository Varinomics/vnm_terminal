...
function(vnm_terminal_expect_qsg_atlas_cursor_report json_text source_path report_name)
    vnm_terminal_read_json_type(
        report_type
        "${json_text}"
        "${source_path}"
        qsg_atlas ${report_name})
    if(NOT report_type STREQUAL "OBJECT")
        message(FATAL_ERROR
            "${source_path} JSON field 'qsg_atlas.${report_name}' should be an object, "
            "got ${report_type}")
    endif()
    ...
    vnm_terminal_expect_json_boolean(
        "${json_text}"
        "${source_path}"
        qsg_atlas ${report_name} valid)
    vnm_terminal_expect_json_boolean(
        "${json_text}"
        "${source_path}"
        qsg_atlas ${report_name} visible)
    vnm_terminal_expect_json_number(
        "${json_text}"
        "${source_path}"
        qsg_atlas ${report_name} shape)
    vnm_terminal_expect_json_number(
        "${json_text}"
        "${source_path}"
        qsg_atlas ${report_name} row)
    vnm_terminal_expect_json_number(
        "${json_text}"
        "${source_path}"
        qsg_atlas ${report_name} column)
endfunction()
...
if(NOT retained_history_estimate_contract_version STREQUAL "2" AND
    NOT retained_history_estimate_contract_version STREQUAL "3")
    message(FATAL_ERROR
        "retained_history.prefix_plain_ascii_estimate.contract_version should be 2 or 3, got "
...
foreach(cursor_report IN ITEMS
    captured_snapshot_cursor
    captured_render_cursor
    emitted_cursor)
    vnm_terminal_expect_qsg_atlas_cursor_report(
        "${metrics_text}"
        "${metrics_path}"
        ${cursor_report})
endforeach()
