#define VNM_TERMINAL_EXAMPLE_TERMINAL_NO_MAIN
#include "../../examples/terminal_app/main.cpp"
#undef VNM_TERMINAL_EXAMPLE_TERMINAL_NO_MAIN
...
namespace chrome_test = vnm_terminal::example_terminal;
...
QPointF button_center(
    const chrome_test::Example_window_chrome&  titlebar,
...
class Focus_window_chrome final : public chrome_test::Example_window_chrome
{
public:
    using chrome_test::Example_window_chrome::Example_window_chrome;
...
protected:
    void invoke_window_command(chrome_test::Window_chrome_command command) override
    {
        ...
        {
            chrome_test::Example_window_chrome::invoke_window_command(command);
        }
    }
};
...
bool test_custom_titlebar_geometry()
{
    ...
    chrome_test::Example_window_chrome titlebar(window.contentItem());
...
bool test_parse_custom_titlebar_option()
{
    Parse_result default_result = parse_arguments({
        QStringLiteral("vnm_terminal_example_terminal"),
        QStringLiteral("--"),
    ...
    Parse_result result = parse_arguments({
        QStringLiteral("vnm_terminal_example_terminal"),
...
#if defined(_WIN32) || defined(__linux__)
    ok &= check(result.error.isEmpty(),
        "custom-titlebar option parses on validated platform");
    ok &= check(result.options.custom_titlebar,
        "custom-titlebar option enables custom titlebar mode");
#else
    ok &= check(!result.error.isEmpty(),
        "custom-titlebar option is rejected on unvalidated platforms");
    ok &= check(!result.options.custom_titlebar,
        "custom-titlebar option does not enable mode on unvalidated platforms");
#endif
...
int main(int argc, char** argv)
{
    ...
    ok &= test_parse_custom_titlebar_option();
