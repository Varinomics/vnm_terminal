    ...
    const VNM_TerminalSurface& surface)
{
    ...
    // size. Until then all geometry persistence remains suppressed.
    ...
    App_options*                           options)
{
    ...
    if (state.position.has_value() &&
        window_geometry_has_useful_visible_area(*state.position, options->window_size))
    {
...
Persisted_interaction_settings load_persisted_interaction_settings(QSettings& settings)
{
...
{
    constexpr char k_settings_no_persist_env[] =
        "VNM_TERMINAL_SETTINGS_NO_PERSIST";
    return
